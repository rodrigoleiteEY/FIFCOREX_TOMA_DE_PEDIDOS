 ...
 * - [ProductsCreateFragment]: to create or edit a(n) Products,
 * - [ProductsDetailFragment]: to display the details of a(n) Products,
 ...
 * what to do.
 */
class ProductsActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<Products> {
    ...
    override fun onFragmentStateChange(evt: Int, entity: Products?) {
        when (evt) {
    ...
    private fun onItemClicked(entity: Products?) {
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: Products?) {
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity).get(ProductsViewModel::class.java)

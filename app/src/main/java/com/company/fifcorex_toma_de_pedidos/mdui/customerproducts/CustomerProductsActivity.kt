 ...
 * - [CustomerProductsCreateFragment]: to create or edit a(n) CustomerProducts,
 * - [CustomerProductsDetailFragment]: to display the details of a(n) CustomerProducts,
 * - [CustomerProductsListFragment]: to list CustomerProducts.
 *
 ...
 * what to do.
 */
class CustomerProductsActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<CustomerProducts> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, CustomerProductsListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: CustomerProducts?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link CustomerProductsCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: CustomerProducts?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, CustomerProductsDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, CustomerProductsDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = CustomerProductsCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: CustomerProducts?) {
        val updateFragment = CustomerProductsCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as CustomerProductsActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity).get(CustomerProductsViewModel::class.java)

 ...
 * - [CustomerPromotionsCreateFragment]: to create or edit a(n) CustomerPromotions,
 * - [CustomerPromotionsDetailFragment]: to display the details of a(n) CustomerPromotions,
 * - [CustomerPromotionsListFragment]: to list CustomerPromotions.
 *
 ...
 * what to do.
 */
class CustomerPromotionsActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<CustomerPromotions> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, CustomerPromotionsListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: CustomerPromotions?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link CustomerPromotionsCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: CustomerPromotions?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, CustomerPromotionsDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, CustomerPromotionsDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = CustomerPromotionsCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: CustomerPromotions?) {
        val updateFragment = CustomerPromotionsCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as CustomerPromotionsActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity).get(CustomerPromotionsViewModel::class.java)

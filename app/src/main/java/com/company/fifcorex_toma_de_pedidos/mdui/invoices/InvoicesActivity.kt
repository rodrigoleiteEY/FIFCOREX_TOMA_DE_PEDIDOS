 ...
 * - [InvoicesCreateFragment]: to create or edit a(n) Invoices,
 * - [InvoicesDetailFragment]: to display the details of a(n) Invoices,
 * - [InvoicesListFragment]: to list Invoices.
 *
 ...
 * what to do.
 */
class InvoicesActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<Invoices> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, InvoicesListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: Invoices?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link InvoicesCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: Invoices?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, InvoicesDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, InvoicesDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = InvoicesCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: Invoices?) {
        val updateFragment = InvoicesCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as InvoicesActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity).get(InvoicesViewModel::class.java)

 ...
 * - [OrdersHeaderCreateFragment]: to create or edit a(n) OrdersHeader,
 * - [OrdersHeaderDetailFragment]: to display the details of a(n) OrdersHeader,
 * - [OrdersHeaderListFragment]: to list OrdersHeader.
 *
 ...
 * what to do.
 */
class OrdersHeaderActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<OrdersHeader> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, OrdersHeaderListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: OrdersHeader?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link OrdersHeaderCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: OrdersHeader?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, OrdersHeaderDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, OrdersHeaderDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = OrdersHeaderCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: OrdersHeader?) {
        val updateFragment = OrdersHeaderCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as OrdersHeaderActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity).get(OrdersHeaderViewModel::class.java)

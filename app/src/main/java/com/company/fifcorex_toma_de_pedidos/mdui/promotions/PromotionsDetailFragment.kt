...
import com.company.fifcorex_toma_de_pedidos.databinding.FragmentPromotionsDetailBinding
...
/**
 * A fragment representing a single Promotions detail screen.
 * This fragment is contained in an PromotionsActivity.
 */
class PromotionsDetailFragment : InterfacedFragment<Promotions>() {

    /** Generated data binding class based on layout file */
    private lateinit var binding: FragmentPromotionsDetailBinding

    /** Promotions entity to be displayed */
    ...
    private fun onDeleteComplete(result: OperationResult<Promotions>) {
        ...
        listener?.onFragmentStateChange(UIConstants.EVENT_DELETION_COMPLETED, promotionsEntity)
    }
    ...
    private fun setupDataBinding(inflater: LayoutInflater, container: ViewGroup?): View {
        binding = FragmentPromotionsDetailBinding.inflate(inflater, container, false)
     ...
     */
    private fun setDetailImage(objectHeader: ObjectHeader, promotionsEntity: Promotions) {
        if (promotionsEntity.getOptionalValue(Promotions.grupoMaterial) != null && promotionsEntity.getOptionalValue(Promotions.grupoMaterial).toString().isNotEmpty()) {
            objectHeader.detailImageCharacter = promotionsEntity.getOptionalValue(Promotions.grupoMaterial).toString().substring(0, 1)
        } else {
    ...
    /**
     * Setup ObjectHeader with an instance of promotionsEntity
     */
    private fun setupObjectHeader() {
        val secondToolbar = currentActivity.findViewById<Toolbar>(R.id.secondaryToolbar)
        if (secondToolbar != null) {
            secondToolbar.setTitle(promotionsEntity.entityType.localName)
        } else {
            currentActivity.setTitle(promotionsEntity.entityType.localName)
        }
        ...
        objectHeader?.let {
            it.apply {
                ...
                subheadline = EntityKeyUtil.getOptionalEntityKey(promotionsEntity)
            ...
            setDetailImage(it, promotionsEntity)
